...
namespace FollowUP.Infrastructure.Services
{
    public class AesEncryptor : IAesEncryptor
    {
        ...
        public AesEncryptor(AesSettings settings)
        {
            _settings = settings;
            _password = GetPassword(_settings.Password);
        }
        ...
        public string Encrypt(string text, string salt)
        {
            ...
            {
                throw new ServiceException(ErrorCodes.TextIsEmpty,
        ...
        private static SecureString GetPassword(string stringPassword)
        {
            var password = new SecureString();

            for(int i = 0; i < stringPassword.Length; i++)
            {
                password.AppendChar(stringPassword[i]);
            }
